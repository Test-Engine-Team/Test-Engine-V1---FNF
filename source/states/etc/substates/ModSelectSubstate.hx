...
class ModSelectSubstate extends MusicBeatSubstate {
	...
	public function new() {
		...
		modList = [
			for (mod in FileSystem.readDirectory("./mods"))
				if (FileSystem.isDirectory("./mods/" + mod) && mod != "+BASE+") mod
		];
		...
		FlxTween.tween(coolEpicAndCoolGradient, {x: 1280 - coolEpicAndCoolGradient.width}, 0.5, {
	...
	override function update(elapsed:Float) {
		...
		switch ([
			...
			(controls.DOWN_P && canSelect),
			(controls.ACCEPT && canSelect),
			(controls.BACK && canLeave)
		].indexOf(true)) {
			case 0:
			...
			case 3:
				canLeave = false;
				canSelect = false;
				FlxTween.tween(coolEpicAndCoolGradient, {x: 1280}, 0.5, {
					ease: FlxEase.circOut,
					onComplete: function(twn:FlxTween) {
						close();
					}
