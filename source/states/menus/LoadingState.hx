...
class LoadingState extends MusicBeatState {
	...
	override public function create() {
		...
		if (!addedCrash) { // Parse default mod data.
			...
			if (defaultXml.get("titleBarName") != null)
				daModData.titleBar = defaultXml.get("titleBarName");
		...
		if (Assets.exists("assets/modData.xml")) {
			...
			if (xml.get("titleBarName") != null)
				modData.titleBar = xml.get("titleBarName");
	...
	function parseDiaFormat(xmlFormat:Xml) {
		...
		if (xmlFormat.get("y") != null)
			format.y = Std.parseFloat(xmlFormat.get("y"));
		if (xmlFormat.get("textx") != null)
			format.textX = Std.parseFloat(xmlFormat.get("textx"));
		if (xmlFormat.get("texty") != null)
			format.textY = Std.parseFloat(xmlFormat.get("texty"));
		...
		if (elements != null && elements.hasNext()) {
			for (element in elements) {
				switch (element.nodeName) {
					case "boxSprite":
						if (element.get("spritePath") != null)
							format.boxSpritePath = element.get("spritePath");
						if (element.get("scale") != null)
							format.boxScale = Std.parseFloat(element.get("scale"));
						if (element.get("antialiasing") != null)
							format.boxAntialiasing = element.get("antialiasing") == "true";
					case "anim":
						format.anims.push(addCharAnim(element));
					case "textFormat":
						if (element.get("font") != null)
							format.font = element.get("font");
						if (element.get("size") != null)
							format.textSize = Std.parseInt(element.get("size"));
						if (element.get("fieldWidth") != null)
							format.textWidth = Std.parseInt(element.get("fieldWidth"));
						if (element.get("color") != null)
							format.textColor = CoolUtil.stringColor(element.get("color"));

						if (element.get("border") != null)
							format.borderEnabled = element.get("border") == "true";
						if (element.get("borderColor") != null)
							format.borderColor = CoolUtil.stringColor(element.get("borderColor"));

						if (element.get("shadow") != null)
							format.dropShadow = element.get("shadow") == "true";
						if (element.get("shadowColor") != null)
							format.shadowColor = CoolUtil.stringColor(element.get("shadowColor"));
				}
			}
		}
	...
	function parseDiaPortait(xmlPortait:Xml) {
		...
		if (xmlPortait.get("image") != null)
			portait.imagePath = xmlPortait.get("image");
		if (xmlPortait.get("flipX") != null)
			portait.flipX = xmlPortait.get("flipX") == "true";
		if (xmlPortait.get("antialiasing") != null)
			portait.antialiasing = xmlPortait.get("antialiasing") == "true";
		if (xmlPortait.get("scale") != null)
			portait.scale = Std.parseFloat(xmlPortait.get("scale"));
		if (xmlPortait.get("xOffset") != null)
			portait.xOffset = Std.parseFloat(xmlPortait.get("xOffset"));
		if (xmlPortait.get("yOffset") != null)
			portait.yOffset = Std.parseFloat(xmlPortait.get("yOffset"));
	...
	function parseCharList(xml:Xml) {
		...
		for (char in xml.elementsNamed("char")) {
			...
			if (char.get("spriteImage") != null)
				modChar.spriteImage = char.get("spriteImage");
			if (char.get("iconImage") != null)
				modChar.iconImage = char.get("iconImage");
			if (char.get("regCharType") != null)
				modChar.regCharType = char.get("regCharType");

			var offsetArray:Array<String> = [];
			if (char.get("offsets") != null)
				offsetArray = char.get("offsets").split(",");
			if (offsetArray.length > 1) {
				for (i in 0...2)
					modChar.offsets[i] = Std.parseFloat(offsetArray[i].trim());
			}
			offsetArray = [];
			if (char.get("camOffsets") != null)
				offsetArray = char.get("camOffsets").split(",");
			if (offsetArray.length > 1) {
				for (i in 0...2)
					modChar.offsets[i + 2] = Std.parseFloat(offsetArray[i].trim());
			}

			if (char.get("scale") != null)
				modChar.scale = Std.parseFloat(char.get("scale"));
			if (char.get("singDur") != null)
				modChar.singDur = Std.parseFloat(char.get("singDur"));

			if (char.get("scaleAffectsOffset") != null)
				modChar.scaleAffectsOffset = (char.get("scaleAffectsOffset") == "true");
			if (char.get("flipX") != null)
				modChar.flipX = (char.get("flipX") == "true");
			if (char.get("antialiasing") != null)
				modChar.antialiasing = (char.get("antialiasing") == "true");
	...
	function addCharAnim(anim:Xml) {
		...
		if (anim.get("name") != null)
			charAnim.name = anim.get("name");
		if (anim.get("prefix") != null)
			charAnim.prefix = anim.get("prefix");
		if (anim.get("looped") != null)
			charAnim.looped = (anim.get("looped") == "true");
		if (anim.get("fps") != null)
			charAnim.fps = Std.parseInt(anim.get("fps"));

		var offsetArray:Array<String> = [];
		if (anim.get("offsets") != null)
			offsetArray = anim.get("offsets").split(",");
		if (offsetArray.length > 1) {
		...
		if (anim.get("indices") != null)
			indiceArray = anim.get("indices").split(",");
	...
	function addModWeek(week:Xml) {
		...
		if (week.get("name") != null)
			modWeek.name = week.get("name");
		if (week.get("image") != null)
			modWeek.spriteImage = week.get("image");
		if (week.get("songs") != null)
			modWeek.songs = [for (song in week.get("songs").split(",")) song.trim()];
		if (week.get("songPaths") != null)
			modWeek.paths = [for (path in week.get("songPaths").split(",")) path.trim()];
		if (week.get("icons") != null)
			modWeek.icons = [for (icon in week.get("icons").split(",")) icon.trim()];
		if (week.get("diffs") != null)
			modWeek.diffs = [for (diff in week.get("diffs").split(",")) diff.trim()];
	...
	function addWeekChar(charXml:Xml) {
		...
		if (charXml.get("path") != null)
			charData.spritePath = charXml.get("path");
		if (charXml.get("idle") != null)
			charData.idleAnim = charXml.get("idle");
		if (charXml.get("confirm") != null)
			charData.confirmAnim = charXml.get("confirm");
		if (charXml.get("scale") != null)
			charData.scale = Std.parseFloat(charXml.get("scale"));
		if (charXml.get("xOffset") != null)
			charData.xOffset = Std.parseFloat(charXml.get("xOffset"));
		if (charXml.get("yOffset") != null)
			charData.yOffset = Std.parseFloat(charXml.get("yOffset"));
		if (charXml.get("flipX") != null)
			charData.flipX = (charXml.get("flipX") == "true");
	...
	function errorPopup(error:UncaughtErrorEvent) {
		...
		var errorMessage:String = switch ([
			Std.isOfType(error.error, openfl.errors.Error),
			Std.isOfType(error.error, openfl.events.ErrorEvent),
			true
		].indexOf(true)) {
		...
		for (stackItem in haxe.CallStack.exceptionStack(true)) {
			switch (stackItem) {
				case CFunction:
					message += "Called from C Function";
				case Module(module):
					message += 'Called from $module (Module)';
				case FilePos(parent, file, line, col):
					message += 'Called from $file on line $line';
				case LocalFunction(func):
					message += 'Called from $func (Local Function)';
				case Method(clas, method):
					message += 'Called from $clas - $method()';
			}
			message += "\n";
		}
