...
class UpdateState extends MusicBeatState {
	...
	public function new(baseURL:String = "http://raw.githubusercontent.com/Test-Engine-Team/Test-Engine-Latest/main/", fileList:Array<String>) {
		super();
	...
	function doFile() {
		...
		while (label1.length < maxLength)
			label1 = " " + label1;
		while (label2.length < maxLength)
			label2 += " ";
		...
		downloadStream.addEventListener(Event.COMPLETE, function(e) {
			var array = [];
			var dir = [
				for (k => e in (array = f.replace("\\", "/").split("/"))) if (k < array.length - 1) e
			].join("/");
