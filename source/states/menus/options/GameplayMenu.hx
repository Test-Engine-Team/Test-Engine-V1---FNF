			...
			updateFunc: function(menuOption:MenuOption, elapsed:Float) {
				switch ([FlxG.keys.justPressed.LEFT, FlxG.keys.justPressed.RIGHT].indexOf(true)) {
					case 0:
						...
						if (ClientPrefs.framerate < menuOption.min)
							ClientPrefs.framerate = Std.int(menuOption.min);
					...
					case 1:
						ClientPrefs.framerate += 10;
						if (ClientPrefs.framerate > menuOption.max)
							ClientPrefs.framerate = Std.int(menuOption.max);
		...
		},
		{
			...
			updateFunc: function(menuOption:MenuOption, elapsed:Float) {
				if (FlxG.keys.justPressed.LEFT) {
					...
					if (ClientPrefs.safeFrames < menuOption.min)
						ClientPrefs.safeFrames = Std.int(menuOption.min);
				} else if (FlxG.keys.justPressed.RIGHT) {
					ClientPrefs.safeFrames += 1;
					if (ClientPrefs.safeFrames > menuOption.max)
						ClientPrefs.safeFrames = Std.int(menuOption.max);
				}
				if (FlxG.keys.justPressed.R)
