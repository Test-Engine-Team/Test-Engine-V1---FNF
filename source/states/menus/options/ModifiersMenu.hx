		...
		},
		{
			...
			updateFunc: function(menuOption:MenuOption, elapsed:Float) {
				if (FlxG.keys.justPressed.LEFT) {
					ClientPrefs.speed -= 1;
					if (ClientPrefs.speed < menuOption.min)
						ClientPrefs.speed = Std.int(menuOption.min);
				} else if (FlxG.keys.justPressed.RIGHT) {
					ClientPrefs.speed += 1;
					if (ClientPrefs.speed > menuOption.max)
						ClientPrefs.speed = Std.int(menuOption.max);
				}
				if (FlxG.keys.justPressed.R)
					ClientPrefs.speed = 1;
			},
